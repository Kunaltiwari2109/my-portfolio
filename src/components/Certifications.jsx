...
const certificates = [
  {
    ...
    date: 'August 31, 2024',
    image: '/java-certificate.pdf', 
    link: '/java-certificate.pdf',
                ...
                <div className="w-full h-40 bg-backgroundSecondary rounded-lg mb-6 overflow-hidden relative flex items-center justify-center border border-white/10 group-hover:border-electricBlue/30 transition-colors">
                  {cert.title.includes('Java') ? (
                    <div className="flex flex-col items-center gap-2">
                      <Award size={48} className="text-neonPurple opacity-50 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300" />
                      <span className="text-xs text-electricBlue/80 font-medium tracking-widest uppercase">View Certificate PDF</span>
                    </div>
                  ) : (
                    <span className="text-textSecondary/50 text-sm">IMAGE PLACEHOLDER</span>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-backgroundPrimary/80 to-transparent pointer-events-none"></div>
                </div>

                <h3 className="text-xl font-bold text-white mb-2 group-hover:text-electricBlue transition-colors">{cert.title}</h3>
                <p className="text-textSecondary text-sm mb-4">{cert.issuer}</p>
                
                {cert.credentialId && (
                  <p className="text-xs text-textSecondary/70 font-mono mb-4">ID: {cert.credentialId}</p>
                )}
