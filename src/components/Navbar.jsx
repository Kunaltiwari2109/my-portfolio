  ...
  return (
    <header
      className={`fixed top-0 w-full z-40 transition-all duration-300 ${
        scrolled
          ? "py-4 glassmorphism shadow-[0_4px_30px_rgba(0,0,0,0.1)]"
          : "py-6 bg-transparent"
      }`}
    >
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-backgroundPrimary/95 backdrop-blur-xl flex flex-col items-center justify-center gap-8 z-40">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
